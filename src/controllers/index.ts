...
export const postTransaction = async (req: any, res: any) => {
    try {
        ...
        if (!Object.values(TransactionTypes).includes(type)) {
            return res
                .status(400)
                .send(`Error in parameter "type". Valid options are { send, receive }, but received: ${req.body.type}`);
        }
...
export const postTransfer = async (req: any, res: any) => {
    try {
        ...
        if (!amount || amount < 0) {
            return res
                .status(400)
                .send(`Error in parameter "number". Must be a positive number but received ${req.body.amount}`);
        }
