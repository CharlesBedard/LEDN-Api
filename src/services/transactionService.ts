...
export async function createTransactionWithAccountUpdate(email: string, amount: number, type: string) {
...
export async function createTransferWithAccountUpdates(senderEmail: string, recipientEmail: string, amount: number) {
    // start a session in order for the user/transaction updates to be "atomic"
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
